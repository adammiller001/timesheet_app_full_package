...
c1, c2 = st.columns(2)
with c1:
    job_choice = st.selectbox("Job Number – Area Number – Description", job_display, index=job_index if "job_index" in locals() else 0, key="job_choice")
with c2:
    code_choice = st.selectbox("Cost code - Description", cost_display, index=cost_index if "cost_index" in locals() else 0, key="cost_choice")
...
if not entered_df.empty:
    ...
    delete_choice = st.multiselect("Choose what to delete", delete_options, default=[], key="delete_choices")
    choices = st.session_state.get("delete_choices", [])
if st.button("Delete", type="primary"):
    if not choices:
        st.warning("Select one or more employees, or choose 'Delete All'.")
    else:
        if "Delete All" in choices:
            entered_df = entered_df.iloc[0:0]  # clear all rows
        else:
            entered_df = entered_df[~entered_df["Employee"].isin(choices)]
        st.session_state["entered_df"] = entered_df
