...
@pytest.fixture
def string_list():
    return ["abc", "def", "ghi" * 10, "AÂ¢â˜ƒâ‚¬ ˜Š" * 100, "Abc" * 1000, "DEF"]
...
@pytest.mark.parametrize(
    ...
    [
        ["this", "is", "an", "array"],
        ["â‚¬", "", "˜Š"],
        ["AÂ¢â˜ƒâ‚¬ ˜Š", " Aâ˜ƒâ‚¬Â¢˜Š", "â˜ƒâ‚¬˜Š AÂ¢", "˜Šâ˜ƒAÂ¢ â‚¬"],
    ],
)
...
def test_replace_non_default_repeat(count):
    a = np.array(["--", "¦œ-¦œ-"], "T")
    result = np.strings.replace(a, "¦œ-", "¦œâ€ ", count)
    assert_array_equal(result, np.array(["--", "¦œâ€ ¦œâ€ "], "T"))
