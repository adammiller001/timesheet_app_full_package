...
def slice(a, start=None, stop=None, step=None, /):
    ...
    >>> b = np.array(['hello world', 'Î³ÎµÎ¹Î± ÏƒÎ¿Ï… ÎºÏŒÏƒÎ¼Îµ', 'ä½ å¥½ä¸–ç•Œ', '‘‹ Œ'],
    ...              dtype=np.dtypes.StringDType())
    >>> np.strings.slice(b, -2)
    array(['hello wor', 'Î³ÎµÎ¹Î± ÏƒÎ¿Ï… ÎºÏŒÏƒ', 'ä½ å¥½', '‘‹'], dtype=StringDType())

    >>> np.strings.slice(b, [3, -10, 2, -3], [-1, -2, -1, 3])
    array(['lo worl', ' ÏƒÎ¿Ï… ÎºÏŒÏƒ', 'ä¸–', '‘‹ Œ'], dtype=StringDType())

    >>> np.strings.slice(b, None, None, -1)
    array(['dlrow olleh', 'ÎµÎ¼ÏƒÏŒÎº Ï…Î¿Ïƒ Î±Î¹ÎµÎ³', 'ç•Œä¸–å¥½ä½ ', 'Œ ‘‹'],
          dtype=StringDType())
